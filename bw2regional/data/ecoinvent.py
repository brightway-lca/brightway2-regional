...
def expand_ecoinvent_location_codes(database):
    assert database in databases, "`database` must be name of a Database"
    for ds in pyprind.ProgBar(Database(database)):
        if ds.get('location') in location_correspondence:
            ds['location'] = location_correspondence[ds['location']]
            ds.save()
