...
def discretize_rest_of_world(database, warn=True):
    ...
    row_locations = {k: [x for x in v if x != 'RoW'] for k, v in locations.items() if 'RoW' in v}

    # check_location_not_repeated_twice(locations):
    for k, v in locations.items():
        assert len(v) == len(set(v)), \
            "Locations repeated in market {}: {}".format(k, v)

    unique_rows = {obj: "RoW-{}".format(index)
        for index, obj in enumerate(
            {frozenset(locs) for locs in locations.values()}
        )
    }
